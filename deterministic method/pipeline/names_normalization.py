        ...
        for i, each_row in enumerate(file_reader):
            ...
            if i % 2 == 1:
                ...
                for method in range(1, 6):
                    similarity = measure_string_distance(
                        fir_name, sec_name, method)
                    sim_max = max(similarity, sim_max)

                if sim_max > threshold:
                    matched_results.append(
                        (fir_id, sec_id))  # append as a tuple
...
def main():
    ...
    threshold = 0.8
    tasks_args = [("process_first_name.csv", 1, threshold, "process_first_name_222.txt"),
                  ("process_last_name.csv", 2, threshold, "process_last_name_222.txt")]
