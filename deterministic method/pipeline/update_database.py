...
engine = create_engine(SQLALCHEMY_DATABASE_URI, pool_size=5, pool_recycle=3600, pool_reset_on_return='commit')
#logging setup
...
def create_new_sql(**param):
	...
	sql = "insert into pcm_update (enterpriseid, address) values ('{}', '{}')".format(_id, new_value)
	return sql
...
def create_new_sql_dob(**param):
	...
	day = param.get("day")
	sql = "insert into pcm_update (enterpriseid, address, year, month, day) values ('{}','{}','{}','{}','{}')".format(_id, new_value, year, month, day)
	return sql
...
def create_update_sql(**param):
	...
	new_value = param.get("value")
	sql = "update pcm_update set {} = '{}' where ENTERPRISEID = '{}'".format(field, new_value, _id)
	return sql
...
def create_update_sql_dob(**param):
	...
	sql = "update pcm_update set {} = '{}', year = '{}', month = '{}', day = '{}' where ENTERPRISEID = '{}'".format(field, new_value, year, month, day, _id)
	return sql
	# _id = param.get("_id")
	# year = int(param.get("year"))
	# month = int(param.get("month"))
	...
	with engine.begin() as conn:
		...
		try:
			...
			if dob is not None:
				...
				conn.execute(create_update_sql_dob(_id=_id, field='naddr', value=naddr, year=year, month=month, day=day))
			else:
				conn.execute(create_update_sql(_id=_id, field='naddr', value=naddr))
	...
	with engine.begin() as conn:
		...
		with concurrent.futures.ThreadPoolExecutor(max_workers=8) as excutor:
			for i, each in enumerate(res):
				...
				future_.add_done_callback(functools.partial(write_out, file=out_file))
				futures_.append(future_)
			concurrent.futures.wait(futures_)
...
def create_sql_and_add_to_file(each):
	...
	if dob is not None:
		year, month, day = dob.strftime("%Y-%m-%d").split("-")
		sql += create_new_sql_dob(_id=_id, field='naddr', value=naddr, year=year, month=month, day=day)
	else:
...
def test():
	...
	with engine.begin() as conn:
		...
		for each in res:
			...
			name = each['name']
			pts = each['pts']
			conn.execute(test_create_test_update_sql("npts", None, "team", team))
